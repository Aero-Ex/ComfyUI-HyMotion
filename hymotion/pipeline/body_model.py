...
class WoodenMesh(torch.nn.Module):
    ...
    def __init__(self, model_path=None):
        ...
        if model_path is None:
            import os
            model_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "assets", "dump_wooden")
    ...
    def forward_batch(self, params, chunk_size=16):
        """
        Process batched motion data in chunks to avoid OOM.
        
        Args:
            params: dict with 'rot6d' (B, T, J, 6) and 'trans' (B, T, 3)
            chunk_size: number of frames to process at once (lower = less VRAM)
        """
